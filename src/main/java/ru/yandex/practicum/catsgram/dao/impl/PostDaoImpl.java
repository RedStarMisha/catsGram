...
public class PostDaoImpl implements PostDao {
    ...
    public Collection<Post> findPostsByUser(User user) {
        String sql = "select * from cat_post where author_id = ? order by creation_date desc ";  //Обратите внимание: в запросе была применена сортировка по дате создания (order by creation_date desc).
